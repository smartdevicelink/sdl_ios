...
describe(@"Getter/Setter Tests", ^ {
    ...
    it(@"Should set and get correctly", ^ {
        ...
        testStruct.softButtonCapabilities = @[button1];
        
        expect(testStruct.windowID).to(equal(@444));
        
        expect(testStruct.testTextField.firstObject.name).to(equal(@"test text field"));
        expect(testStruct.testImageField.firstObject.name).to(equal(@"test text field"));
        ...
        expect(testStruct.buttonCapabilities.firstObject.name).to(equal(@YES));
